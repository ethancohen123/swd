...
def pyramid_down(image,channels, device="cpu"):
    gaussian_k = get_gaussian_kernel(device=device)        
    # channel-wise conv(important)
    multiband = [F.conv2d(image[:, i:i + 1,:,:], gaussian_k, padding=2, stride=2) for i in range(channels)]
    down_image = torch.cat(multiband, dim=1)
    return down_image

def pyramid_up(image, channels, device="cpu"):
    gaussian_k = get_gaussian_kernel(device=device)
    upsample = F.interpolate(image, scale_factor=2)
    multiband = [F.conv2d(upsample[:, i:i + 1,:,:], gaussian_k, padding=2) for i in range(channels)]
    up_image = torch.cat(multiband, dim=1)
    return up_image

def gaussian_pyramid(original, n_pyramids,channels, device="cpu"):
    x = original
    # pyramid down
    pyramids = [original]
    for i in range(n_pyramids):
        x = pyramid_down(x,channels, device=device)
        pyramids.append(x)
    return pyramids

def laplacian_pyramid(original, n_pyramids,channels, device="cpu"):
    # create gaussian pyramid
    pyramids = gaussian_pyramid(original, n_pyramids,channels, device=device)

    # pyramid up - diff
    laplacian = []
    for i in range(len(pyramids) - 1):
        diff = pyramids[i] - pyramid_up(pyramids[i + 1],channels, device=device)
        laplacian.append(diff)
    # Add last gaussian pyramid
    laplacian.append(pyramids[len(pyramids) - 1])        
    return laplacian

def minibatch_laplacian_pyramid(image, n_pyramids, batch_size,channels, device="cpu"):
    n = image.size(0) // batch_size + np.sign(image.size(0) % batch_size)
    pyramids = []
    for i in range(n):
        x = image[i * batch_size:(i + 1) * batch_size]
        p = laplacian_pyramid(x.to(device), n_pyramids,channels, device=device)
        p = [x.cpu() for x in p]
        pyramids.append(p)
    del x
    result = []
...
def extract_patches(pyramid_layer, slice_indices,channels,
    ...
    x = (x - mean) / (std + 1e-8)
    # reshape to 2rank
    x = x.reshape(-1, channels * slice_size * slice_size)
    return x
        
def swd(image1, image2, channels,
    ...
    with torch.no_grad():
        ...
        pyramid1 = minibatch_laplacian_pyramid(image1, n_pyramids, pyramid_batchsize,channels, device=device)
        pyramid2 = minibatch_laplacian_pyramid(image2, n_pyramids, pyramid_batchsize, channels,device=device)
        result = []
        ...
        for i_pyramid in range(n_pyramids + 1):
            ...
            p1 = extract_patches(pyramid1[i_pyramid], indices,channels,
                            slice_size=slice_size, device="cpu")
            p2 = extract_patches(pyramid2[i_pyramid], indices,channels,
                            slice_size=slice_size, device="cpu")
